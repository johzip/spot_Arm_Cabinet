...
@configclass
class SpotCabinetEnvCfg(DirectRLEnvCfg):
    # env
    decimation = 5
    ...
    cabinet_cfg: ArticulationCfg = ArticulationCfg(
        ...
        init_state=ArticulationCfg.InitialStateCfg(
            pos=(0.0, 0.0, 0.39146906),
            rot=(0.0673854, 0, 0, -0.997727),  # x, y, z, w
            joint_pos={"drawer_joint": 0.0},  # Initial drawer position
        ),
    )
...
class SpotCabinetEnv( DirectRLEnv):
    cfg: SpotCabinetEnvCfg
    def __init__(self,
                 cfg: SpotCabinetEnvCfg,
    ...
    def _reset_idx(self, env_ids):
        ...
        if hasattr(self, 'cabinet') and self.cabinet is not None:
            # Reset cabinet to initial state
            cabinet_pos = torch.zeros((num_indices, self.cabinet.num_joints), device=self.sim.device)
            cabinet_vel = torch.zeros((num_indices, self.cabinet.num_joints), device=self.sim.device)
            
            # Set initial drawer position
            drawer_joint_idx = self.cabinet.find_joints("drawer_joint")[0] if self.cabinet.find_joints("drawer_joint") else None
            if drawer_joint_idx is not None:
                cabinet_pos[:, drawer_joint_idx] = 0.0  # Closed position
            
            self.cabinet.write_joint_state_to_sim(cabinet_pos, cabinet_vel, None, env_ids)
